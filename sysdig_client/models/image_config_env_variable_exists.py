...
class ImageConfigEnvVariableExists(BaseModel):
    """
    Predicate expressing \"the container has the specified environment variable\". 
    """ # noqa: E501
    type: StrictStr
    extra: ImageConfigEnvVariableExistsExtra
    __properties: ClassVar[List[str]] = ["type", "extra"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['imageConfigEnvVariableExists']):
            raise ValueError("must be one of enum values ('imageConfigEnvVariableExists')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "extra": ImageConfigEnvVariableExistsExtra.from_dict(obj["extra"]) if obj.get("extra") is not None else None
        })
        return _obj
