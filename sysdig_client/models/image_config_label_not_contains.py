...
class ImageConfigLabelNotContains(BaseModel):
    """
    Predicate expressing \"the image has the specified label and value\". 
    """ # noqa: E501
    type: StrictStr
    extra: ImageConfigLabelNotContainsExtra
    __properties: ClassVar[List[str]] = ["type", "extra"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['imageConfigLabelNotContains']):
            raise ValueError("must be one of enum values ('imageConfigLabelNotContains')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "extra": ImageConfigLabelNotContainsExtra.from_dict(obj["extra"]) if obj.get("extra") is not None else None
        })
        return _obj
