...
if ok:
    ...
    dataToPrint = " ".join(
        [
            str(x["id"]).ljust(colLen) if len(str(x["id"])) < colLen else str(x["id"])[: (colLen - 3)].ljust(colLen - 3) + "..."
            for x in metrics
        ]
    )
    ...
    for d in data:
        ...
        dataToPrint = " ".join(
            [str(x).ljust(colLen) if len(str(x)) < colLen else str(x)[: (colLen - 3)].ljust(colLen - 3) + "..." for x in values]
        )
