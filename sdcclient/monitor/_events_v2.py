...
class EventsClientV2(_SdcCommon):
    ...
    def get_events(
        self, name=None, category=None, direction="before", status=None, limit=100, pivot=None, from_s=None, to_s=None
    ):
        """**Description**
    ...
    def delete_event(self, event):
        ...
        **Example**
        ...
        res = self.http.delete(self.url + "/api/v2/events/" + str(event["id"]), headers=self.hdrs, verify=self.ssl_verify)
    ...
    def post_event(self, name, description=None, severity=None, event_filter=None, tags=None):
        ...
        **Examples**
        ...
        """
        options = {"name": name, "description": description, "severity": severity, "scope": event_filter, "tags": tags}
        edata = {"event": {k: v for k, v in options.items() if v is not None}}
        res = self.http.post(self.url + "/api/v2/events/", headers=self.hdrs, data=json.dumps(edata), verify=self.ssl_verify)
        return self._request_result(res)
