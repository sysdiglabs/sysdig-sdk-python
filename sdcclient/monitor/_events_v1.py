...
class EventsClientV1(_SdcCommon):
    ...
    def post_event(self, name, description=None, severity=None, event_filter=None, tags=None):
        ...
        **Examples**
        ...
        """
        options = {"name": name, "description": description, "severity": severity, "filter": event_filter, "tags": tags}
        edata = {"event": {k: v for k, v in options.items() if v is not None}}
        res = self.http.post(self.url + "/api/events/", headers=self.hdrs, data=json.dumps(edata), verify=self.ssl_verify)
        return self._request_result(res)
