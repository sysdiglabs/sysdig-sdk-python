...
with description("Dashboards v3", "integration") as self:
    ...
    with it("is able to create a dashboard from a file"):
        ...
        with tempfile.NamedTemporaryFile(mode="w+") as f:
            ...
            ok, res = self.client.create_dashboard_from_file(
                dashboard_name=f"{_DASHBOARD_NAME}_2", filename=f.name, filter=None
            )
            expect((ok, res)).to(be_successful_api_call)
    ...
    with it("is able to create a dashboard from a view"):
        ...
        call = self.client.create_dashboard_from_view(
            newdashname=f"{_DASHBOARD_NAME}_2",
    ...
    with context("when there are existing dashbords"):
        ...
        with it("is able to retrieve the test dashboard by its id"):
            ...
            expect(res).to(
                have_key(
                    "dashboard", have_keys("name", id=equal(self.test_dashboard["id"]))
                )
            )
        ...
        with context("when deleting a dashboard"):
            ...
            with it(
                "returns an error if there is not 'id' field in the provided object"
            ):
        ...
        with it("is able to dump the dashboard to a file"):
            with tempfile.NamedTemporaryFile(mode="w+") as f:
                self.client.save_dashboard_to_file(
                    dashboard=self.test_dashboard, filename=f.name
                )
                f.flush()
                f.seek(0)
        ...
        with context("when it's created with an incorrect scope"):
            with it("fails if the scope is not a string"):
                ...
                expect(ok).to(be_false)
                expect(res).to(
                    equal("Invalid scope format: Expected a list, a string or None")
                )
        ...
        with context("when creating a dashboard from other dashboard"):
            ...
            with it("returns an error saying the dashboard does not exist"):
                ok, res = self.client.create_dashboard_from_dashboard(
                    newdashname=f"{_DASHBOARD_NAME}_2",
                    templatename="NonExistingDashboard",
                    filter=None,
                )
                expect(ok).to(be_false)
                expect(res).to(
                    equal(
                        "can't find dashboard NonExistingDashboard to use as a template"
                    )
                )
            ...
            expect(res).to(
                contain(
                    have_key(
                        "dashboard",
                        have_keys(
                            id=self.test_dashboard["id"],
                            name=self.test_dashboard["name"],
                        ),
                    )
                )
            )
        ...
        with context("when we are sharing a dashboard with all teams"):
            with it("shares it with view only permissions"):
                ok, res = self.client.share_dashboard_with_all_teams(
                    self.test_dashboard, "r"
                )
                ...
                expect(res["dashboard"]["sharingSettings"][0]["role"]).to(
                    equal("ROLE_RESOURCE_READ")
                )

            with it("shares it with read write permissions"):
                ok, res = self.client.share_dashboard_with_all_teams(
                    self.test_dashboard, "w"
                )
                ...
                expect(res["dashboard"]["sharingSettings"]).to(have_len(1))
                expect(res["dashboard"]["sharingSettings"][0]["role"]).to(
                    equal("ROLE_RESOURCE_EDIT")
                )

        with context("when there is a shared dashboard"):
            with it("unshares it"):
                _, dboard = self.client.share_dashboard_with_all_teams(
                    self.test_dashboard, "w"
                )
        ...
        with context("when we are sharing a dashboard with a particular team"):
            ...
            with it("shares it with view only permissions"):
                ...
                ok, res = self.client.share_dashboard_with_team(
                    self.test_dashboard, team["id"], "r"
                )
            ...
            with it("shares it with two teams, one of those with write access"):
                ...
                ok_team, res_team = self.client.share_dashboard_with_team(
                    self.test_dashboard, team["id"], "r"
                )
                ok_team2, res_team2 = self.client.share_dashboard_with_team(
                    res_team["dashboard"], self.team["team"]["id"], "w"
                )
                ...
                expect(res_team2["dashboard"]["sharingSettings"]).to(
                    contain(have_keys(role=equal("ROLE_RESOURCE_READ")))
                )
                expect(res_team2["dashboard"]["sharingSettings"]).to(
                    contain(have_keys(role=equal("ROLE_RESOURCE_EDIT")))
                )
