...
class NewStage(BaseModel):
    ...
    configuration: Optional[Annotated[List[StageConfiguration], Field(min_length=0, max_length=1000)]] = Field(default=None, description="Configurations for the stage")
    __properties: ClassVar[List[str]] = ["name", "configuration"]

    @field_validator('name')
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['runtime', 'pipeline', 'registry']):
            raise ValueError("must be one of enum values ('runtime', 'pipeline', 'registry')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "name": obj.get("name"),
            "configuration": [StageConfiguration.from_dict(_item) for _item in obj["configuration"]] if obj.get("configuration") is not None else None
        })
        return _obj
