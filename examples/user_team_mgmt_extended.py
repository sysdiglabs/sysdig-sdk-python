...
try:
    ok, res = sdclient.create_user_invite(
        admin, first_name="TestUser", last_name="Admin", system_role="ROLE_CUSTOMER"
    )
    if not ok:
    ...
    ok, res = sdclient.create_user_invite(
        userA, first_name="TestUser", last_name="Alpha"
    )
    if not ok:
        print(("-- User creation failed:", res, ". Exiting."))
        sys.exit(1)
    ...
    ok, res = sdclient.create_user_invite(
        userB, first_name="TestUser", last_name="Beta"
    )
    if not ok:
        print(("-- User creation failed:", res, ". Exiting."))
        sys.exit(1)
    ...
    if not ok:
        print(
            ("-- Unable to add ", userA, " to ", teamA, " due to: ", res, ". Exiting.")
        )
        sys.exit(1)
    ...
    ok, res = sdclient.save_memberships(
        teamB, {userA: "ROLE_TEAM_MANAGER", userB: "ROLE_TEAM_READ"}
    )
    if not ok:
        print(
            (
    ...
    ok, res = sdclient.save_memberships(
        teamA, {userA: "ROLE_TEAM_READ", userB: "ROLE_TEAM_EDIT"}
    )
    if not ok:
        print(
            (
                "-- Unable to modify membership for ",
                userA,
                " and to add ",
                userB,
                " to ",
    ...
    if not ok:
        ...
        print(
            (
                ...
                " and ",
                userB,
                " should have appropriate roles assigned for team ",
                teamA,
                "!",
                "Exiting.",
            )
    ...
    if not ok:
        print(
            (
                "-- Unable to remove membership for ",
                userB,
                " from team",
                teamA,
                " due to: ",
    ...
    if not ok:
        ...
        print(
            ("-- User ", userB, " should not be part of team ", teamA, "!", "Exiting.")
        )
        sys.exit(1)
    ...
    if not ok:
        ...
        print(
            ("-- User ", userA, " should not be part of team ", teamB, "!", "Exiting.")
        )
        sys.exit(1)
    elif admin not in list(res.keys()):
