...
with description("Scanning Alerts", "integration") as self:
    ...
    with it("lists all the scanning alerts"):
        ok, res = self.client.add_runtime_alert(
            name="A name", description="A description", scope="", triggers=[SdScanningClient.RuntimeAlertTrigger.unscanned_image]
        )
        expect((ok, res)).to(be_successful_api_call)
    ...
    with context("when creating a runtime alert"):
        ...
        with it("creates an alert with scan result change trigger as 'Pass > Fail'"):
            ...
            expect(res).to(
                have_keys(
                    ...
                    description="A description",
                    scope="",
                    triggers=have_keys(policy_eval=be_true),
                    onlyPassFail=be_true,
                )
            )
        ...
        with it("creates an alert with scan result change trigger as 'Any Change'"):
            ...
            expect(res).to(
                have_keys(
                    ...
                    scope="",
                    triggers=have_keys(policy_eval=be_true),
                    onlyPassFail=be_false,
                )
            )

        with it("creates an alert with cve update trigger"):
            ok, res = self.client.add_runtime_alert(
                name="A name", description="A description", scope="", triggers=[SdScanningClient.RuntimeAlertTrigger.cve_update]
            )

            expect((ok, res)).to(be_successful_api_call)
            expect(res).to(
                have_keys(name="A name", description="A description", scope="", triggers=have_keys(vuln_update=be_true))
            )
        ...
        with it("creates an alert with multiple triggers"):
            ok, res = self.client.add_runtime_alert(
                ...
                scope="",
                triggers=[
                    SdScanningClient.RuntimeAlertTrigger.unscanned_image,
            ...
            expect(res).to(
                have_keys(
                    ...
                    description="A description",
                    scope="",
                    triggers=have_keys(unscanned=be_true, policy_eval=be_true, vuln_update=be_true),
                    onlyPassFail=be_true,
                )
            )
    ...
    with context("when creating a repository alert"):
        with it("creates an alert with new image analyzed trigger"):
            ...
            expect((ok, res)).to(be_successful_api_call)
            expect(res).to(
                have_keys(name="A name", description="A description", scope="", triggers=have_keys(analysis_update=be_true))
            )
        ...
        with it("creates an alert with multiple triggers"):
            ok, res = self.client.add_repository_alert(
                ...
                description="A description",
                triggers=[
                    SdScanningClient.RepositoryAlertTrigger.new_image_analyzed,
            ...
            expect(res).to(
                have_keys(
                    ...
                    description="A description",
                    scope="",
                    triggers=have_keys(analysis_update=be_true, policy_eval=be_true, vuln_update=be_true),
                    onlyPassFail=be_true,
                )
            )
    ...
    with it("updates a runtime alert correctly"):
        ...
        expect(res).to(
            have_keys(
                ...
                description="An updated description",
                scope="agent.id = 'foo'",
                triggers=have_keys(unscanned=be_false, policy_eval=be_true),
                onlyPassFail=be_true,
            )
        )
