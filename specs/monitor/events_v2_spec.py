...
with description("Events v2", "integration") as self:
    ...
    with it("is able to retrieve events by name"):
        ...
        expect((ok, res)).to(be_successful_api_call)
        expect(res).to(
            have_key("events", contain(have_key("name", equal(self.event_name))))
        )
    ...
    with context("but the from and to parameters are incorrectly specified"):
        with it(
            "returns an error if any of the parameters is specified but not the other"
        ):
            ...
            expect(res1).to(
                equal(
                    "only one of 'from_s' or 'to_s' has been specified, "
