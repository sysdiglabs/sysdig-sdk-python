...
class SsoSettingsResponseV1(BaseModel):
    ...
    is_active: Optional[StrictBool] = Field(
        default=None, description="Flag to indicate if the SSO settings is active.", alias="isActive"
    )
    create_user_on_login: Optional[StrictBool] = Field(
        default=None,
        description="Flag to indicate if the user will be created automatically if not found in the system.",
        alias="createUserOnLogin",
    )
    is_password_login_enabled: Optional[StrictBool] = Field(
        default=None,
    ...
    is_single_logout_enabled: Optional[StrictBool] = Field(
        default=None, description="Flag to indicate if the single logout support is enabled.", alias="isSingleLogoutEnabled"
    )
    is_group_mapping_enabled: Optional[StrictBool] = Field(
        default=None, description="Flag to indicate if group mapping support is enabled.", alias="isGroupMappingEnabled"
    )
    group_mapping_attribute_name: Optional[Annotated[str, Field(strict=True, max_length=64)]] = Field(
        default=None, description="The group mapping attribute name.", alias="groupMappingAttributeName"
    )
    ...
    date_created: Optional[datetime] = Field(
        default=None, description="The date (in ISO 8601 format) when this SSO config was created.", alias="dateCreated"
    )
    last_updated: Optional[datetime] = Field(
        default=None, description="The date (in ISO 8601 format) when this SSO config was last updated.", alias="lastUpdated"
    )
