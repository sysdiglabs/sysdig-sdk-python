...
class VulnEpssPercentileGte(BaseModel):
    """
    The EPSS percentile score that is greater than or equal to the specified value.
    """ # noqa: E501
    type: StrictStr
    extra: VulnEpssPercentileGteExtra
    __properties: ClassVar[List[str]] = ["type", "extra"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['vulnEpssPercentileGte']):
            raise ValueError("must be one of enum values ('vulnEpssPercentileGte')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "extra": VulnEpssPercentileGteExtra.from_dict(obj["extra"]) if obj.get("extra") is not None else None
        })
        return _obj
