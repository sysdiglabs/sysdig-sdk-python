        ...
        with it("creates an alert with unscanned image trigger"):
            ...
            expect(res).to(
                have_keys(
                    ...
                    description="A description",
                    scope="",
                    triggers=have_keys(unscanned=be_true),
                )
            )
        ...
        with it("creates an alert with cve update trigger"):
            ...
            expect(res).to(
                have_keys(
                    ...
                    description="A description",
                    scope="",
                    triggers=have_keys(vuln_update=be_true),
                )
            )
        ...
        with it("creates an alert with multiple triggers"):
            ...
            expect(res).to(
                have_keys(
                    ...
                    scope="",
                    triggers=have_keys(
                        unscanned=be_true, policy_eval=be_true, vuln_update=be_true
                    ),
                    onlyPassFail=be_true,
                )
            )
    ...
    with context("when creating a repository alert"):
        ...
        with it("creates an alert with scan result change trigger as 'Any Change'"):
            ok, res = self.client.add_repository_alert(
                ...
                description="A description",
                triggers=[
                    SdScanningClient.RepositoryAlertTrigger.scan_result_change_any
                ],
            )
    ...
    with it("updates a repository alert correctly"):
        ...
        expect(res).to(
            have_keys(
                name="A name",
                description="A description",
                repositories=contain_exactly(
                    have_keys(
                        registry="registry", repository="repository", tag="latest"
                    )
                ),
                triggers=have_keys(analysis_update=be_true),
            )
        ...
        expect(res).to(
            have_keys(
                ...
                repositories=contain_exactly(
                    have_keys(
                        registry="new_registry", repository="new_repository", tag="v1"
                    )
                ),
                triggers=have_keys(unscanned=be_false, policy_eval=be_true),
                onlyPassFail=be_true,
