...
from sdcclient.monitor.dashboard_converters._dashboard_scope import (
...
def _convert_dashboard_v1_to_v2(dashboard):
    ...
    def convert_events_filter(prop_name, old_dashboard, new_dashboard):
        ...
        if (
            ...
            in new_dashboard["eventsOverlaySettings"]
        ):
            del new_dashboard["eventsOverlaySettings"][
                "showNotificationsDoNotFilterSameMetrics"
            ]
        if (
    ...
    def convert_items(prop_name, old_dashboard, new_dashboard):
        def convert_color_coding(prop_name, old_widget, new_widget):
            ...
            if best_value is not None and worst_value is not None:
                new_widget[prop_name] = {"best": best_value, "worst": worst_value}
        ...
        def convert_group(prop_name, old_widget, new_widget):
            group_by_metrics = old_widget[prop_name]["configuration"]["groups"][0][
                "groupBy"
            ]
        ...
        def convert_metrics(prop_name, old_widget, new_widget):
            ...
            timestamp_key = [
                m
                for m in migrated_metrics
                if m["id"] == "timestamp"
                and "timeAggregation" not in m
                or not (m["timeAggregation"] is not None)
            ]
            no_timestamp_keys = [
                m
                for m in migrated_metrics
            ...
            values = [
                m
                for m in migrated_metrics
                if "timeAggregation" in m and m["timeAggregation"] is not None
            ]
            if timestamp_key:
    ...
    migrations = {
        ...
        "eventsFilter": with_default(
            convert_events_filter, {"filterNotificationsUserInputFilter": ""}
        ),
...
_DASHBOARD_CONVERTERS = {"v2": {"v1": _convert_dashboard_v1_to_v2}}
...
def convert_dashboard_between_versions(dashboard, version_from, version_to):
    ...
    **Success Return Value**
    ...
    if converters_to is None:
        return (
            False,
            f"unexpected error: no dashboard converters from version {version_to} are supported",
        )

    converter = converters_to.get(version_from, None)

    if converter is None:
        return False, "dashboard version {} cannot be converted to {}".format(
