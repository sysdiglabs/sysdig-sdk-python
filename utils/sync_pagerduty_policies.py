...
def run(sysdig_token, pager_duty_id, pager_duty_token, link, unlink, dry_run):
    ...
    #
    sysdig_vendor = pager_duty.get(
        "/vendors", {"query": "sysdig", "limit": 1, "offset": 0, "total": "false"}
    )["vendors"][0]
    ...
    for service in services:
        for integration in service["integrations"]:
            integration["details"] = pager_duty.get(
                "/services/{}/integrations/{}".format(service["id"], integration["id"])
            )["integration"]
    ...
    if unlink:
        ...
        for channel in pager_duty_channels:
            actions.append(
                {
                    "info": 'Sysdig: Delete channel "{}" ({})'.format(
        ...
        for service in services:
            if service["sysdig_integrations"]:
                if len(service["sysdig_integrations"]) == len(service["integrations"]):
                    ...
                    actions.append(
                        {
                            "info": 'PagerDuty: Delete service "{}" ({})'.format(
    ...
    if link:
        ...
        for channel in pager_duty_channels:
            if channel["options"]["serviceKey"] not in service_integration_keys:
                actions.append(
                    {
                        "info": 'Remove notification channel "{}" not connected to any integration'.format(
                            channel["name"]
                        ),
                        "fn": actions_factory.delete_notification_channel(channel),
                    }
        ...
        for policy in escalation_policies:
            ...
            policy_services = [
                service
                for service in services
                if service["escalation_policy"]["id"] == policy["id"]
            ]
            sysdig_services = [
                service for service in policy_services if service["sysdig_integrations"]
            ]
            ...
            if not sysdig_services:
                ...
                actions.append(
                    {
                        "info": 'Create service, integration, and notification channel for policy "{}"'.format(
                            policy["name"]
                        ),
                    ...
                    for integration in service["integrations"]:
                        if (
                                ...
                                actions.append(
                                    {
                                        ...
                                        "fn": actions_factory.rename_notification_channel(
                                            channel, policy["name"], service_name
                                        ),
                                    }
                                )
                            ...
                            if (
                                ...
                                actions.append(
                                    {
                                        "info": 'Rename service "{}" to "{}"'.format(
                                            service["name"], service_name
                                        ),
                                        "fn": actions_factory.rename_service(
...
class PagerDutyAPI:
    ...
    def _base_request(self, method, endpoint, params=None, data=None):
        ...
        response = getattr(requests, method)(
            url, params=params, data=request_data, headers=self._get_headers()
        )
...
class ActionFactory:
    ...
    def delete_integration(self, service_id, integration_id):
        def fn():
            self._pager_duty.delete(
                "/services/{}/integrations/{}".format(service_id, integration_id)
            )
