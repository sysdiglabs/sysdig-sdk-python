...
class UpdateTeamRequestV1(BaseModel):
    """
    UpdateTeamRequestV1
    """  # noqa: E501

    version: Annotated[int, Field(strict=True, ge=0)] = Field(description=" ")
    name: Annotated[str, Field(strict=True, max_length=256)] = Field(description="The name of the team.")
    description: Optional[Annotated[str, Field(strict=True, max_length=2048)]] = Field(
        default=None, description="A description of the team explaining what is it used for."
    )
    is_default_team: StrictBool = Field(
        description="Specifies if this is the default team. The default team is used to automatically assign new users to a team.",
        alias="isDefaultTeam",
    )
    standard_team_role: Optional[TeamRoleV1] = Field(
        default=None,
        description="The standard team role assigned by default to users added to this team. ",
        alias="standardTeamRole",
    )
    custom_team_role_id: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(
        default=None,
        description="The custom team role assigned by default to users added to this team. ",
        alias="customTeamRoleId",
    )
    ui_settings: UiSettingsV1 = Field(alias="uiSettings")
    is_all_zones: Optional[StrictBool] = Field(
        default=None,
        description="**True** if the users that are members of this team have access to all zones. **Mutually exclusive with zoneIds**.\\ \\ _Only supported in Secure features._ ",
        alias="isAllZones",
    )
    zone_ids: Optional[Annotated[List[Annotated[int, Field(strict=True, ge=0)]], Field(max_length=20)]] = Field(
        default=None,
        description="The list of zones that users assigned to this team will have access to. **Mutually exclusive with allZones**.\\ \\ _Only supported in Secure features._ ",
        alias="zoneIds",
    )
    scopes: Annotated[List[ScopeV1], Field(min_length=1, max_length=512)] = Field(
        description="Scopes is a list of different scope types and filter values that will be applied to resources when accessed through the team. "
    )
    additional_team_permissions: AdditionalTeamPermissionsV1 = Field(alias="additionalTeamPermissions")
        ...
        _obj = cls.model_validate(
            {
                ...
                "additionalTeamPermissions": AdditionalTeamPermissionsV1.from_dict(obj["additionalTeamPermissions"])
                if obj.get("additionalTeamPermissions") is not None
                else None,
            }
        )
        return _obj
