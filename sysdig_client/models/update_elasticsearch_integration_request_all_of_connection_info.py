...
class UpdateElasticsearchIntegrationRequestAllOfConnectionInfo(BaseModel):
    """
    ...
    format: Optional[StrictStr] = Field(
        default=None,
        description='If specified, transforms `content.fields` and `labels` * `KV_PAIRS` - from objects (i.e. `{"foo": "bar", ...}`) to arrays of objects with "key" and "value" properties {i.e. `[{"key" : "foo", "value": "bar"}, ...]`) ',
    )
    secret: Optional[Annotated[str, Field(strict=True, max_length=256)]] = Field(
        default=None, description="Authentication secret. To be set if auth is specified"
    )
