...
parser = argparse.ArgumentParser(
    description="Synchronize PagerDuty escalation policies with Sysdig, "
...
parser.add_argument(
    "--unlink", action="store_true", help="Set to remove notification channels connected to PagerDuty escalation policies"
)
parser.add_argument("--dry-run", action="store_true", help="Set to get a report of changes, without actually apply them")
...
def run(sysdig_token, pager_duty_id, pager_duty_token, link, unlink, dry_run):
    ...
    print(
        "Found {} PagerDuty notification {} configured in Sysdig".format(
    ...
    #
    escalation_policies = pager_duty.get("/escalation_policies")["escalation_policies"]
    print(
        "Found {} PagerDuty escalation {}".format(
            len(escalation_policies), pluralize("policy", len(escalation_policies), "policies")
        )
    )
    ...
    print("Found {} PagerDuty {}".format(len(services), pluralize("service", len(services))))
    # print(json.dumps(services, sort_keys=True, indent=4))

    #
    # Get Sysdig vendor configuration
    #
    sysdig_vendor = pager_duty.get("/vendors", {"query": "sysdig", "limit": 1, "offset": 0, "total": "false"})["vendors"][0]
    ...
    for service in services:
        for integration in service["integrations"]:
            integration["details"] = pager_duty.get("/services/{}/integrations/{}".format(service["id"], integration["id"]))[
                "integration"
            ]
    ...
    for service in services:
        ...
        for integration in service["sysdig_integrations"]:
            service_integration_keys[integration["integration_key"]] = {"service": service, "integration": integration}
                        ...
                        actions.append(
                            {
                                "info": 'PagerDuty: Delete integration "{}" ({}) in service "{}" ({})'.format(
                                    integration["name"], integration["id"], service["name"], service["id"]
                                ),
        ...
        for policy in escalation_policies:
            ...
            for service in sysdig_services:
                for integration in service["integrations"]:
                    if (
                        integration["vendor"]
                        and integration["vendor"]["id"] == sysdig_vendor["id"]
                        and integration["integration_key"] not in integration_keys
                    ):
                        disconnected_services.append({"service": service, "integration": integration})
                ...
                actions.append(
                    {
                        "info": 'Restore notification channel for disconnected service "{}" for policy "{}"'.format(
                            disconnected_services[0]["service"]["name"], policy["name"]
                        ),
                        "fn": actions_factory.create_notification_channel(
                            policy, disconnected_services[0]["service"], disconnected_services[0]["integration"]
                        ),
                    }
                )
            else:
                for service in sysdig_services:
                    for integration in service["integrations"]:
                        if (
                            integration["vendor"]
                            and integration["vendor"]["id"] == sysdig_vendor["id"]
                            and integration["integration_key"] in integration_keys
                        ):
                            ...
                            if channel["name"] != policy["name"]:
                                ...
                                actions.append(
                                    {
                                        "info": 'Rename channel service "{}" to service name "{}"'.format(
...
class ActionFactory:
    ...
    def create_all(self, policy, sysdig_vendor):
        def fn():
            new_service = self._pager_duty.post(
                "/services",
                {
                    "service": {
                        "type": "service",
                        ...
                        "status": "active",
                        "escalation_policy": {"id": policy["id"], "type": "escalation_policy_reference"},
            ...
            new_integration = self._pager_duty.post(
                "/services/{}/integrations".format(new_service["id"]),
                {
                    "integration": {
                        "type": "integration_inbound_integration",
                        "name": "Sysdig",
                        "vendor": {"id": sysdig_vendor["id"], "type": "vendor"},
                        "service": {
                            "id": new_service["id"],
...
run(
    args["sysdig-token"][0],
    args["pagerduty-account-id"][0],
    args["pagerduty-access-key"][0],
    args["link"],
    args["unlink"],
    args["dry_run"],
)
