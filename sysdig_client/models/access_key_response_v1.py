...
class AccessKeyResponseV1(BaseModel):
    ...
    is_enabled: Optional[StrictBool] = Field(
        default=None, description="Indicates if the access key is enabled", alias="isEnabled"
    )
    access_key: Optional[Annotated[str, Field(strict=True, max_length=256)]] = Field(
        default=None, description="Displays the access key value", alias="accessKey"
    )
    date_created: Optional[datetime] = Field(
        default=None, description="Date and time when access key was created ", alias="dateCreated"
    )
    date_disabled: Optional[datetime] = Field(
        default=None, description="Date and time when access key was disabled ", alias="dateDisabled"
    )
    agent_limit: Optional[Annotated[int, Field(le=2147483647, strict=True, ge=0)]] = Field(
        default=None, description="Maximum number of agents that can be connected with the access key", alias="agentLimit"
    )
    agent_reservation: Optional[Annotated[int, Field(le=2147483647, strict=True, ge=0)]] = Field(
        default=None,
        description="Number of agents that are guaranteed to be available for the access key",
        alias="agentReservation",
    )
    team_id: Optional[Annotated[int, Field(le=2147483647, strict=True, ge=0)]] = Field(
        default=None, description="ID of team that owns the access key", alias="teamId"
    )
    metadata: Optional[Dict[str, Annotated[str, Field(strict=True, max_length=76)]]] = Field(
        default=None,
