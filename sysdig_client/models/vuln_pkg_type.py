...
class VulnPkgType(BaseModel):
    """
    Predicate expressing \"the Vulnerability is related to a package of <type>\".
    """  # noqa: E501

    type: StrictStr
    extra: VulnPkgTypeExtra
    __properties: ClassVar[List[str]] = ["type", "extra"]

    @field_validator("type")
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["vulnPkgType"]):
            raise ValueError("must be one of enum values ('vulnPkgType')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {"type": obj.get("type"), "extra": VulnPkgTypeExtra.from_dict(obj["extra"]) if obj.get("extra") is not None else None}
        )
        return _obj
