...
class VulnEpssScoreGte(BaseModel):
    """
    The EPSS score that meets or exceeds the specified threshold value.
    """ # noqa: E501
    type: StrictStr
    extra: VulnEpssScoreGteExtra
    __properties: ClassVar[List[str]] = ["type", "extra"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['vulnEpssScoreGte']):
            raise ValueError("must be one of enum values ('vulnEpssScoreGte')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "extra": VulnEpssScoreGteExtra.from_dict(obj["extra"]) if obj.get("extra") is not None else None
        })
        return _obj
