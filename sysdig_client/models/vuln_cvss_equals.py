...
class VulnCvssEquals(BaseModel):
    """
    Predicate expressing \"the Vulnerability CVSS Score is exactly equal to <value>\" 
    """ # noqa: E501
    type: StrictStr
    extra: VulnCvssEqualsExtra
    __properties: ClassVar[List[str]] = ["type", "extra"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['vulnCVSSEquals']):
            raise ValueError("must be one of enum values ('vulnCVSSEquals')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "extra": VulnCvssEqualsExtra.from_dict(obj["extra"]) if obj.get("extra") is not None else None
        })
        return _obj
