...
class GroupMappingSettingsV1(BaseModel):
    ...
    different_roles_same_team_strategy: StrictStr = Field(
        description="Strategy for handling different roles on the same team. ", alias="differentRolesSameTeamStrategy"
    )
    no_mappings_error_redirect_url: Optional[Annotated[str, Field(strict=True, max_length=2048)]] = Field(
        default=None, description="Redirect URL when NO_MAPPINGS_ERROR_REDIRECT is selected. ", alias="noMappingsErrorRedirectUrl"
    )
    ...
    def no_mapping_strategy_validate_enum(cls, value):
        ...
        if value not in set(["UNAUTHORIZED", "DEFAULT_TEAM_DEFAULT_ROLE", "NO_MAPPINGS_ERROR_REDIRECT"]):
            raise ValueError(
                "must be one of enum values ('UNAUTHORIZED', 'DEFAULT_TEAM_DEFAULT_ROLE', 'NO_MAPPINGS_ERROR_REDIRECT')"
            )
        return value
