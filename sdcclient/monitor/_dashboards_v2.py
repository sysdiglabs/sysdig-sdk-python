...
class DashboardsClientV2(_SdcCommon):
    ...
    def add_dashboard_panel(
        self, dashboard, name, panel_type, metrics, scope=None, sort_direction="desc", limit=None, layout=None
    ):
        """**Description**
        ...
        **Example**
        ...
        if "custom_display_options" not in panel_configuration:
            panel_configuration["custom_display_options"] = {
                "valueLimit": {"count": 10, "direction": "desc"},
                "histogram": {"numberOfBuckets": 10},
                "yAxisScale": "linear",
                "yAxisLeftDomain": {"from": 0, "to": None},
                "yAxisRightDomain": {"from": 0, "to": None},
                "xAxis": {"from": 0, "to": None},
            }
        #
        ...
        if panel_type == "timeSeries":
            ...
            if limit is not None:
                panel_configuration["custom_display_options"]["valueLimit"] = {"count": limit, "direction": "desc"}
            ...
            if limit is not None:
                panel_configuration["custom_display_options"]["valueLimit"] = {"count": limit, "direction": sort_direction}
    ...
    def create_dashboard_from_template(self, dashboard_name, template, scope, shared=False, public=False):
        ...
        if scopeRes:
            template["scopeExpressionList"] = list(
                map(
                    lambda ex: {
                        "operand": ex["operand"],
