...
class _SdcCommon(object):
    ...
    **Arguments**
        ...
        - **sd_ibm_cloud**: Use IBM IAM authentication by passing in a SdIbmCloud object
    ...
    **Returns**
    ...
    def __init__(self, token="", sdc_url='https://app.sysdigcloud.com', ssl_verify=True, sd_ibm_cloud=None):
        self.token = os.environ.get("SDC_TOKEN", token)
        self.url = os.environ.get("SDC_URL", sdc_url)
        self.hdrs = self.__get_headers(sd_ibm_cloud)
    ...
    def __get_headers(self, sd_ibm_cloud):
        headers = {'Content-Type': 'application/json'}
        if sd_ibm_cloud:
            headers['Authorization'] = 'Bearer ' + sd_ibm_cloud.get_iam_token(self.url)
            headers['IBMInstanceID'] = sd_ibm_cloud.guid
        else:
