...
class VulnDisclosureRange(BaseModel):
    """
    The start and end dates (inclusive) when vulnerabilities were disclosed.
    """ # noqa: E501
    type: StrictStr
    extra: VulnDisclosureRangeExtra
    __properties: ClassVar[List[str]] = ["type", "extra"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['vulnDisclosureRange']):
            raise ValueError("must be one of enum values ('vulnDisclosureRange')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "extra": VulnDisclosureRangeExtra.from_dict(obj["extra"]) if obj.get("extra") is not None else None
        })
        return _obj
