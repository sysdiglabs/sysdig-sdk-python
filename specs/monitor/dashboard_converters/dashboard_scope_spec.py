...
with description("Dashboard Scopes"):
    with it("parses correctly: agent.id is foo"):
        ...
        expect(res).to(
            equal(
                [True, [{"displayName": "", "isVariable": False, "operand": "agent.id", "operator": "equals", "value": ["foo"]}]]
            )
        )
    ...
    with it("parses correctly: agent.id is not foo"):
        ...
        expect(res).to(
            equal(
                [
                    True,
                    [{"displayName": "", "isVariable": False, "operand": "agent.id", "operator": "notEquals", "value": ["foo"]}],
                ]
            )
        )
    ...
    with it("parses correctly: agent.id in foo"):
        ...
        res = convert_scope_string_to_expression(param)
        expect(res).to(
            equal([True, [{"displayName": "", "isVariable": False, "operand": "agent.id", "operator": "in", "value": ["foo"]}]])
        )
    ...
    with it("parses correctly: agent.id in [foo, bar]"):
        ...
        expect(res).to(
            equal(
                [
                    True,
                    [{"displayName": "", "isVariable": False, "operand": "agent.id", "operator": "in", "value": ["foo", "bar"]}],
                ]
            )
        )
    ...
    with it("parses correctly: agent.id not in foo"):
        ...
        expect(res).to(
            equal(
                [True, [{"displayName": "", "isVariable": False, "operand": "agent.id", "operator": "notIn", "value": ["foo"]}]]
            )
        )
    ...
    with it("parses correctly: agent.id contains foo"):
        ...
        expect(res).to(
            equal(
                [
                    True,
                    [{"displayName": "", "isVariable": False, "operand": "agent.id", "operator": "contains", "value": ["foo"]}],
                ]
            )
        )
    ...
    with it("parses correctly: agent.id starts with foo"):
        ...
        expect(res).to(
            equal(
                [
                    True,
                    [{"displayName": "", "isVariable": False, "operand": "agent.id", "operator": "startsWith", "value": ["foo"]}],
                ]
            )
        )
