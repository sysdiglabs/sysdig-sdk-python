...
#
ok, res = sdclient.get_notifications(from_ts=int(time.time() - 86400), to_ts=int(time.time()))
...
#
ok, res = sdclient.get_notifications(from_ts=int(time.time() - 86400), to_ts=int(time.time()), state="ACTIVE")
...
# Get the notifications in the last day and active state
#
ok, res = sdclient.get_notifications(from_ts=int(time.time() - 86400), to_ts=int(time.time()), state="OK")
...
#
ok, res = sdclient.get_notifications(from_ts=int(time.time() - 86400), to_ts=int(time.time()), resolved=True)
