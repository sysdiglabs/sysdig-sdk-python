...
class FalcoRulesFilesClientOld(_SdcCommon):
    ...
    def _get_falco_rules(self, kind):
        res = self.http.get(self.url + "/api/settings/falco/{}RulesFile".format(kind), headers=self.hdrs, verify=self.ssl_verify)
    ...
    def set_user_falco_rules(self, rules_content):
        ...
        **Example**
        ...
        res = self.http.put(
            self.url + "/api/settings/falco/customRulesFiles", headers=self.hdrs, data=json.dumps(res), verify=self.ssl_verify
        )
    ...
    def _get_falco_rules_files(self, kind):
        res = self.http.get(self.url + "/api/settings/falco/{}RulesFiles".format(kind), headers=self.hdrs, verify=self.ssl_verify)
        if not self._checkResponse(res):
            return [False, self.lasterr]
        data = res.json()
                    ...
                    with open(cpath, "r") as content_file:
                        try:
                            ...
                            fobj["variants"].append(
                                {"requiredEngineVersion": required_engine_version, "content": content_file.read()}
                            )
                        except ValueError:
