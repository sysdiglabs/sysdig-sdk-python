...
class _SdcCommon(object):
    ...
    **Returns**
    ...
    def create_email_notification_channel(self, channel_name, email_recipients):
        channel_json = {
            "notificationChannel": {
                ...
                "name": channel_name,
                "enabled": True,
                "options": {"emailRecipients": email_recipients},
            }
        }

        res = self.http.post(
            self.url + "/api/notificationChannels", headers=self.hdrs, data=json.dumps(channel_json), verify=self.ssl_verify
        )
        return self._request_result(res)
    ...
    def create_notification_channel(self, channel):
        ...
        channel_json = {"notificationChannel": channel}
    ...
    def update_notification_channel(self, channel):
        ...
        res = self.http.put(
            self.url + "/api/notificationChannels/" + str(channel["id"]),
            headers=self.hdrs,
            data=json.dumps({"notificationChannel": channel}),
    ...
    def delete_notification_channel(self, channel):
        ...
        res = self.http.delete(
            self.url + "/api/notificationChannels/" + str(channel["id"]), headers=self.hdrs, verify=self.ssl_verify
        )
        if not self._checkResponse(res):
            return False, self.lasterr
        return True, None
    ...
    def get_topology_map(self, grouping_hierarchy, time_window_s, sampling_time_s):
        ...
        #
        req_json = {
            "format": {"type": "map", "exportProcess": True},
            "time": timeinfo,
            # 'filter': {
            #    'filters': [
            #        {
            ...
            #    'logic': 'and'
            # },
            "limit": {"hostGroups": 20, "hosts": 20, "containers": 20, "processes": 10},
            "group": {"configuration": {"groups": [{"filters": [], "groupBy": gby}]}},
            "nodeMetrics": [{"id": "cpu.used.percent", "aggregation": "timeAvg", "groupAggregation": "avg"}],
            "linkMetrics": [{"id": "net.bytes.total", "aggregation": "timeAvg", "groupAggregation": "sum"}],
        }
        ...
        #
        res = self.http.post(
            self.url + "/api/data?format=map", headers=self.hdrs, data=json.dumps(req_json), verify=self.ssl_verify
        )
        return self._request_result(res)

    def get_data(self, metrics, start_ts, end_ts=0, sampling_s=0, filter="", datasource_type="host", paging=None):
        """**Description**
        ...
        **Examples**
        ...
        res = self.http.post(self.url + "/api/data/", headers=self.hdrs, data=json.dumps(reqbody), verify=self.ssl_verify)
        return self._request_result(res)
    ...
    def create_user(self, user_email, first_name=None, last_name=None, password=None):
        ...
        res = self.http.post(
            self.url + "/api/user/provisioning/", headers=self.hdrs, data=json.dumps(user_info), verify=self.ssl_verify
        )
        return self._request_result(res)
    ...
    def create_user_invite(self, user_email, first_name=None, last_name=None, system_role=None):
        ...
        **Examples**
        ...
        # Create the user
        options = {"username": user_email, "firstName": first_name, "lastName": last_name, "systemRole": system_role}
        user_json = {k: v for k, v in options.items() if v is not None}

        res = self.http.post(self.url + "/api/users", headers=self.hdrs, data=json.dumps(user_json), verify=self.ssl_verify)
        return self._request_result(res)
    ...
    def edit_user(self, user_email, firstName=None, lastName=None, systemRole=None):
        ...
        res = self.http.put(
            self.url + "/api/users/" + str(user["id"]), headers=self.hdrs, data=json.dumps(reqbody), verify=self.ssl_verify
        )
        if not self._checkResponse(res):
            return [False, self.lasterr]
    ...
    def create_team(
        ...
        theme="#7BB0B2",
        perm_capture=False,
        perm_custom_events=False,
        perm_aws_data=False,
        perm_rapid_response=False,
    ):
        """
        **Description**
            Creates a new team
        ...
        **Example**
        ...
        res = self.http.post(self.url + "/api/teams", headers=self.hdrs, data=json.dumps(reqbody), verify=self.ssl_verify)
        return self._request_result(res)
    ...
    def edit_team(
        ...
        **Example**
        ...
        res = self.http.put(
            self.url + "/api/teams/" + str(team["id"]), headers=self.hdrs, data=json.dumps(reqbody), verify=self.ssl_verify
        )
        return self._request_result(res)
    ...
    def create_access_key(self, agent_limit=None, agent_reserved=None, team_id=None):
        ...
        **Reslut**
        ...
        """
        access_key_payload = {"customerAccessKey": {"limit": agent_limit, "reservation": agent_reserved, "teamId": team_id}}

        res = self.http.post(
            self.url + "/api/customer/accessKeys", headers=self.hdrs, verify=self.ssl_verify, data=json.dumps(access_key_payload)
        )
        return self._request_result(res)
    ...
    def update_access_key(self, access_key, agent_limit=None, agent_reserved=None, team_id=None):
        ...
        **Reslut**
        ...
        res = self.http.put(
            self.url + "/api/customer/accessKeys/" + access_key,
            headers=self.hdrs,
            verify=self.ssl_verify,
    ...
    def disable_access_key(self, access_key):
        ...
        **Reslut**
        ...
        """
        res = self.http.post(
            self.url + "/api/customer/accessKeys/" + access_key + "/disable/", headers=self.hdrs, verify=self.ssl_verify
        )
        return self._request_result(res)
    ...
    def enable_access_key(self, access_key):
        ...
        **Reslut**
        ...
        """
        res = self.http.post(
            self.url + "/api/customer/accessKeys/" + access_key + "/enable/", headers=self.hdrs, verify=self.ssl_verify
        )
        return self._request_result(res)
    ...
    def set_agents_config(self, config):
        res = self.http.put(self.url + "/api/agents/config", headers=self.hdrs, data=json.dumps(config), verify=self.ssl_verify)
        return self._request_result(res)
    ...
    def get_user_api_token(self, username, teamname):
        ...
        res = self.http.get(self.url + "/api/token/%s/%d" % (username, team["id"]), headers=self.hdrs, verify=self.ssl_verify)
        if not self._checkResponse(res):
            return [False, self.lasterr]
        data = res.json()
