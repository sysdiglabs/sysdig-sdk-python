...
class UpdateKafkaIntegrationRequestAllOfConnectionInfo(BaseModel):
    """
    UpdateKafkaIntegrationRequestAllOfConnectionInfo
    """  # noqa: E501

    brokers: Annotated[List[Annotated[str, Field(strict=True, max_length=256)]], Field(max_length=32)] = Field(
        description="Kafka server endpoint. A Kafka cluster may provide several brokers; it follows the 'hostname: port' (without protocol scheme)."
    )
    topic: Annotated[str, Field(strict=True, max_length=256)] = Field(
        description="Kafka topic where you want to store the forwarded data"
    )
    balancer: Optional[StrictStr] = Field(
        default="murmur2",
        description="Algorithm that the client uses to multiplex data between the multiple Brokers. For compatibility with the Java client, Murmur2 is used as the default partitioner",
    )
    ...
    keytab: Optional[Annotated[str, Field(strict=True, max_length=1048576)]] = Field(
        default=None, description="base64 encoded Kerberos keytab"
    )
