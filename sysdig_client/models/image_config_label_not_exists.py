...
class ImageConfigLabelNotExists(BaseModel):
    """
    Predicate expressing \"the image does not have the specified label\". 
    """ # noqa: E501
    type: StrictStr
    extra: ImageConfigLabelNotExistsExtra
    __properties: ClassVar[List[str]] = ["type", "extra"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['imageConfigLabelNotExists']):
            raise ValueError("must be one of enum values ('imageConfigLabelNotExists')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "extra": ImageConfigLabelNotExistsExtra.from_dict(obj["extra"]) if obj.get("extra") is not None else None
        })
        return _obj
