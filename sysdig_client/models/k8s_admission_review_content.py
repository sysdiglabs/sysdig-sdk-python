...
class K8sAdmissionReviewContent(BaseModel):
    ...
    namespace: Optional[Annotated[str, Field(strict=True, max_length=253)]] = Field(
        default=None, description="Kubernetes namespace"
    )
    cluster_name: Optional[Annotated[str, Field(strict=True, max_length=256)]] = Field(
        default=None, description="Kubernetes cluster name", alias="clusterName"
    )
    resource_name: Optional[Annotated[str, Field(strict=True, max_length=253)]] = Field(
        default=None, description="Kubernetes resource name", alias="resourceName"
    )
    resource_kind: Optional[Annotated[str, Field(strict=True, max_length=63)]] = Field(
        default=None, description="Kubernetes resource kind", alias="resourceKind"
    )
    zones: Optional[Annotated[List[Zone], Field(max_length=1000)]] = Field(
        default=None, description="List of zones that match the scope of the resource."
    )
        ...
        _obj = cls.model_validate(
            {
                ...
                "scanResult": K8sAdmissionReviewContentAllOfScanResult.from_dict(obj["scanResult"])
                if obj.get("scanResult") is not None
                else None,
            }
        )
        return _obj
