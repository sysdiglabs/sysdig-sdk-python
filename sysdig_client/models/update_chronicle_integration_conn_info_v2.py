...
class UpdateChronicleIntegrationConnInfoV2(BaseModel):
    """
    Update Chronicle Integration Connection V2 Info
    """  # noqa: E501

    region: Optional[StrictStr] = Field(default="us", description="The target region")
    chronicle_customer_id: Annotated[str, Field(strict=True, max_length=64)] = Field(
        description="Unique identifier (UUID) corresponding to a particular Chronicle instance", alias="chronicleCustomerId"
    )
    namespace: Annotated[str, Field(strict=True, max_length=64)] = Field(
        description="User-configured environment namespace to identify the data domain the logs originated from"
    )
    credentials_o_auth2: Optional[Annotated[str, Field(strict=True, max_length=1048576)]] = Field(
        default=None, description="The Chronicle v2 OAuth2 credentials", alias="credentialsOAuth2"
    )
    ...
    def region_validate_enum(cls, value):
        ...
        if value not in set(
            ...
            raise ValueError(
                "must be one of enum values ('us', 'europe', 'europe-west2', 'europe-west3', 'europe-west6', 'asia-southeast1', 'asia-south1', 'asia-northeast1', 'me-west1', 'me-central2', 'australia-southeast1')"
            )
        return value
