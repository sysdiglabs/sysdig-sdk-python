...
class UpdateGroupMappingRequestV1(BaseModel):
    """
    UpdateGroupMappingRequestV1
    """  # noqa: E501

    group_name: Annotated[str, Field(strict=True, max_length=256)] = Field(
        description="The group name matching the external provider name.", alias="groupName"
    )
    ...
    is_admin: StrictBool = Field(description="Flag that indicates if the group is an admin group.", alias="isAdmin")
    team_map: TeamMapV1 = Field(alias="teamMap")
    weight: Annotated[int, Field(le=32767, strict=True, ge=1)] = Field(
