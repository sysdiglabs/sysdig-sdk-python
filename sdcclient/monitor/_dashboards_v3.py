...
from sdcclient.monitor.dashboard_converters import convert_dashboard_between_versions, convert_scope_string_to_expression
...
class DashboardsClientV3(_SdcCommon):
    ...
    def get_views_list(self):
        res = self.http.get(self.url + self._default_dashboards_api_endpoint, headers=self.hdrs, verify=self.ssl_verify)
    ...
    def get_view(self, name):
        ...
        res = self.http.get(
            self.url + self._default_dashboards_api_endpoint + "/" + id, headers=self.hdrs, verify=self.ssl_verify
        )
    ...
    def get_dashboards(self, light=True):
        ...
        res = self.http.get(self.url + self._dashboards_api_endpoint, params=params, headers=self.hdrs, verify=self.ssl_verify)
        return self._request_result(res)
    ...
    def add_dashboard_panel(self, dashboard, panel_name, visualization, query):
        ...
        new_panel = {
            ...
            "advancedQueries": [
                {
                    "enabled": True,
                    "displayInfo": {"displayName": "", "timeSeriesDisplayNameTemplate": "", "type": "lines"},
                    "format": {"unit": "%", "inputFormat": "0-100", "displayFormat": "auto", "decimals": None, "yAxis": "auto"},
                    "query": query,
                }
            ],
        }
        ...
        if visualization == PANEL_VISUALIZATION_TIMECHART:
            ...
            new_panel["legendConfiguration"] = {"enabled": True, "position": "right", "layout": "table", "showCurrent": True}
    ...
    def create_dashboard_from_template(self, dashboard_name, template, scope=None, shared=False, public=False):
        ...
        if "eventsOverlaySettings" not in template or not template["eventsOverlaySettings"]:
            template["eventsOverlaySettings"] = {"filterNotificationsUserInputFilter": ""}
    ...
    def create_dashboard_from_file(self, dashboard_name, filename, filter=None, shared=False, public=False):
        ...
        **Example**
        ...
        if "dashboard" not in loaded_object:
            loaded_object = {"version": f"v{loaded_object['schema']}", "dashboard": loaded_object}
    ...
    def get_dashboard(self, dashboard_id):
        ...
        **Example**
        ...
        """
        res = self.http.get(
            self.url + self._dashboards_api_endpoint + "/" + str(dashboard_id), headers=self.hdrs, verify=self.ssl_verify
        )
        return self._request_result(res)
    ...
    def create_dashboard_from_dashboard(self, newdashname, templatename, filter=None, shared=False, public=False):
        ...
        **Example**
        ...
        dashboard = self.http.get(
            self.url + self._dashboards_api_endpoint, params={"light": "true"}, headers=self.hdrs, verify=self.ssl_verify
        )
    ...
    def save_dashboard_to_file(self, dashboard, filename):
        ...
        **Example**
        ...
        with open(filename, "w") as outf:
            json.dump({"version": self._dashboards_api_version, "dashboard": dashboard}, outf)
    ...
    def delete_dashboard(self, dashboard):
        ...
        **Example**
        ...
        res = self.http.delete(
            self.url + self._dashboards_api_endpoint + "/" + str(dashboard["id"]), headers=self.hdrs, verify=self.ssl_verify
        )
        if not self._checkResponse(res):
            return [False, self.lasterr]
