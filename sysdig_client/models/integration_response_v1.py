...
class IntegrationResponseV1(BaseModel):
    ...
    customer_id: Annotated[int, Field(strict=True, ge=0)] = Field(
        description="Customer identifier for which the integration was created.", alias="customerId"
    )
    ...
    is_all_teams: StrictBool = Field(
        description="**True** if the integration is enabled for all teams, **false** otherwise and the *teamIds* for which is accessible will be defined. ",
        alias="isAllTeams",
    )
    team_ids: Optional[Annotated[List[Annotated[int, Field(strict=True, ge=0)]], Field(max_length=500)]] = Field(
        default=None, description="The list of internal Sysdig team ids that the integration is enabled for.", alias="teamIds"
    )
    jira_project_id: Annotated[str, Field(strict=True, max_length=512)] = Field(
        description="Jira project id.", alias="jiraProjectId"
    )
    jira_project_name: Optional[Annotated[str, Field(strict=True, max_length=512)]] = Field(
        default=None, description="Jira project name.", alias="jiraProjectName"
    )
    ...
    jira_assignee: Optional[Annotated[str, Field(strict=True, max_length=512)]] = Field(
        default=None, description="The default assignee account id for this integration.", alias="jiraAssignee"
    )
    jira_labels: Optional[Annotated[List[Annotated[str, Field(strict=True, max_length=512)]], Field(max_length=500)]] = Field(
        default=None, description="The list of labels available for this integration.", alias="jiraLabels"
    )
    parent_issue_type: Optional[IssueTypeResponseV1] = Field(
        default=None, description="Parent issue type for the integration.", alias="parentIssueType"
    )
    child_issue_type: Optional[IssueTypeResponseV1] = Field(
        default=None, description="Child issue type for the integration.", alias="childIssueType"
    )
        ...
        _obj = cls.model_validate(
            {
                ...
                "parentIssueType": IssueTypeResponseV1.from_dict(obj["parentIssueType"])
                if obj.get("parentIssueType") is not None
                else None,
                "childIssueType": IssueTypeResponseV1.from_dict(obj["childIssueType"])
                if obj.get("childIssueType") is not None
                else None,
            }
        )
        return _obj
