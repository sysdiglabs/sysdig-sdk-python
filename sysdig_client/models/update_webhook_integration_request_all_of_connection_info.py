...
class UpdateWebhookIntegrationRequestAllOfConnectionInfo(BaseModel):
    """
    UpdateWebhookIntegrationRequestAllOfConnectionInfo
    """  # noqa: E501

    endpoint: Annotated[str, Field(strict=True, max_length=128)] = Field(
        description="Webhook endpoint following the schema protocol"
    )
    is_insecure: Optional[StrictBool] = Field(default=False, description="Skip TLS certificate verification", alias="isInsecure")
    headers: Optional[Annotated[List[KeyValueObject], Field(max_length=256)]] = Field(
        default=None,
        description="Extra headers to add to the request. Each header mapping requires 2 keys: 'key' for the header key and 'value' for its value",
    )
    output: Optional[StrictStr] = Field(default="json", description="Payload format")
    timestamp_format: Optional[StrictStr] = Field(
        default="nanoseconds", description='The resolution of the "timestamp" field in the payload', alias="timestampFormat"
    )
    ...
    secret: Optional[Annotated[str, Field(strict=True, max_length=128)]] = Field(
        default=None, description='Secret to use, according to the "auth" value.'
    )
        ...
        _obj = cls.model_validate(
            {
                ...
                "headers": [KeyValueObject.from_dict(_item) for _item in obj["headers"]]
