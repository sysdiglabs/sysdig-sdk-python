...
class ImageConfigCreationDateWithAge(BaseModel):
    """
    Predicate expressing \"the image has been created before the specified amount of days\". 
    """ # noqa: E501
    type: StrictStr
    extra: ImageConfigCreationDateWithAgeExtra
    __properties: ClassVar[List[str]] = ["type", "extra"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['imageConfigCreationDateWithAge']):
            raise ValueError("must be one of enum values ('imageConfigCreationDateWithAge')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "extra": ImageConfigCreationDateWithAgeExtra.from_dict(obj["extra"]) if obj.get("extra") is not None else None
        })
        return _obj
